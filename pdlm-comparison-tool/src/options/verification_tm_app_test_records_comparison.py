...
def run_comparison():       
    st.markdown("### Verification TM APP x Test Records Comparison")
    st.info("Upload your TM APP and Verification Test Records files to compare test protocols.")
    col1, col2 = st.columns(2)
    with col1:
        tm_file = st.file_uploader("Upload TM APP (.xlsx)", type=["xlsx"], key="tmapp_ver2")
    with col2:
        tp_file = st.file_uploader("Upload Verification Test Records (.xlsx)", type=["xlsx"], key="tp_ver2")
    if tm_file and tp_file:
        if st.button("🔍 Run Comparison", key="run_ver_tp_tm"):
            ...
            tm_df = tm_df.rename(columns=lambda x: x.strip())
            tm_df = tm_df[['Verification Test ID', 'Verification Test Result (Pass/Fail)', 'Verification Version']].dropna(subset=['Verification Test ID'])
            tm_df['Verification Test ID'] = tm_df['Verification Test ID'].astype(str).str.strip()


            # Read both sheets from Verification Test Records
            xl_tp = pd.ExcelFile(tp_path)
            sheets_to_read = ["Test Case Report - URS MD", "Test Case Report - URS NMD"]
            test_case_list = []
            traceability_list = []# Adicione estas listas para as colunas extras:
            conclusion_list = []
            version_tested_list = []
            ...
            for sheet in sheets_to_read:
                if sheet in xl_tp.sheet_names:
                    ...
                    # Adicione aqui:
                    if "Test Case " in df.columns and "Conclusion (Pass / Fail)" in df.columns and "Version tested" in df.columns:
                        conclusion_list.append(df[["Test Case ", "Conclusion (Pass / Fail)"]].dropna(subset=["Test Case "]))
                        version_tested_list.append(df[["Test Case ", "Version tested"]].dropna(subset=["Test Case "]))
                else:
                    st.warning(f"Sheet '{sheet}' not found in the uploaded Verification Test Protocol file.")


                # Combine and clean
            TRcol = pd.concat(test_case_list).drop_duplicates()
            TRprs = pd.concat(traceability_list).drop_duplicates()
            TRcol = normalize_spaces(filter_ignored(TRcol, IGNORE_SENTENCES_VAL_TP_TM))
            TRprs = normalize_spaces(filter_ignored(TRprs, IGNORE_SENTENCES_VAL_TP_TM))
            
            # Combine as colunas extras em DataFrames
            if conclusion_list:
                conclusion_df = pd.concat(conclusion_list, ignore_index=True)
                conclusion_df = conclusion_df.rename(columns=lambda x: x.strip())
                # Use o nome sem espaço
                if 'Test Case' in conclusion_df.columns:
                    conclusion_df['Test Case'] = conclusion_df['Test Case'].astype(str).str.strip()
                elif 'Test Case ' in conclusion_df.columns:
                    conclusion_df = conclusion_df.rename(columns={'Test Case ': 'Test Case'})
                    conclusion_df['Test Case'] = conclusion_df['Test Case'].astype(str).str.strip()
                else:
                    st.error("Column 'Test Case' not found in conclusion_df.")
            else:
                conclusion_df = pd.DataFrame(columns=["Test Case", "Conclusion (Pass / Fail)"])

            if version_tested_list:
                version_tested_df = pd.concat(version_tested_list, ignore_index=True)
                version_tested_df = version_tested_df.rename(columns=lambda x: x.strip())
                if 'Test Case' in version_tested_df.columns:
                    version_tested_df['Test Case'] = version_tested_df['Test Case'].astype(str).str.strip()
                elif 'Test Case ' in version_tested_df.columns:
                    version_tested_df = version_tested_df.rename(columns={'Test Case ': 'Test Case'})
                    version_tested_df['Test Case'] = version_tested_df['Test Case'].astype(str).str.strip()
                else:
                    st.error("Column 'Test Case' not found in version_tested_df.")
            else:
                version_tested_df = pd.DataFrame(columns=["Test Case", "Version tested"])

            # Test Case comparison
            tc_results = {
                'Verification Test Case Only in Traceability Application': sorted(set(TMcol) - set(TRcol)),
                'Verification Test Case Only in Verification Test Records': sorted(set(TRcol) - set(TMcol)),
                'Verification Test Case Common to Both Documents': sorted(set(TMcol) & set(TRcol))
            }
            # PRS comparison
            prs_results = {
                'Product Requirements Only in Traceability Application': sorted(set(TMprs) - set(TRprs)),
                'Product Requirements Only in Verification Test Records': sorted(set(TRprs) - set(TMprs)),
                'Product Requirements Common to Both Documents': sorted(set(TMprs) & set(TRprs))
            }
            ...
            # Merge pelo Test Case ID
            merged = pd.merge(
                tm_df,
                conclusion_df,
                left_on='Verification Test ID',
                right_on='Test Case',  # sem espaço!
                how='inner',
                suffixes=('_TM', '_TR')
            )
            # Comparação dos resultados
            result_diff = merged[
                merged['Verification Test Result (Pass/Fail)'] != merged['Conclusion (Pass / Fail)']
            ][['Verification Test ID', 'Verification Test Result (Pass/Fail)', 'Conclusion (Pass / Fail)']]
            result_diff = result_diff.drop_duplicates(subset=['Verification Test ID'])

            # Merge para versão
            merged_version = pd.merge(
                tm_df,
                version_tested_df,
                left_on='Verification Test ID',
                right_on='Test Case',  # sem espaço!
            ...
            version_diff = merged_version[
                merged_version['Verification Version'] != merged_version['Version tested']
            ][['Verification Test ID', 'Verification Version', 'Version tested']]
            version_diff = version_diff.drop_duplicates(subset=['Verification Test ID'])

            st.write("Test Cases with different Test Result (TM APP vs Test Records)")
            st.dataframe(result_diff, use_container_width=True)

            st.write("Test Cases with different Version (TM APP vs Test Records)")
            st.dataframe(version_diff, use_container_width=True)
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer) as writer:
                tc_df.to_excel(writer, sheet_name='Test Case COMP', index=False)
                prs_df.to_excel(writer, sheet_name='PRS COMP', index=False)
                result_diff.to_excel(writer, sheet_name='Test Result Diff', index=False)
                version_diff.to_excel(writer, sheet_name='Version Diff', index=False)
            buffer.seek(0)
            st.success("Comparison complete! Download your results below.")
            st.download_button(
                "⬇️ Download Excel",
                data=buffer,
                file_name="VER_TRxTMAPP_T_comparison_summary.xlsx",
