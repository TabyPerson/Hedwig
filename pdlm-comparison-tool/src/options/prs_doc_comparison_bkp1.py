...
def get_feature_ids_from_word(doc_path, col_name='PRS'):
    ...
    feature_ids = []
    # Modo de leitura: None, 'matrix', 'annexes'
    reading_mode = None
    section_matrix = normalize_text('13. Annex 1 – Matrix of PRS and TCs')
    section_annexes = normalize_text('10. Annexes / Appendices')
    elements = list(doc.element.body)
    table_idx = 0
    def extract_prs_ids(text):
        return [x.strip() for x in re.findall(r'TASY_PRS_ID_[\d\.]+', text) if x.strip()]
    for el in elements:
        if el.tag.endswith('p'):
            ...
            # Ativa/desativa modo de leitura
            if section_matrix in norm_text:
                reading_mode = 'matrix'
            elif section_annexes in norm_text:
                reading_mode = 'annexes'
            # Se encontrar outro título de seção, desativa modo de leitura
            elif (reading_mode is not None and (norm_text.startswith('annex') or norm_text.startswith('appendix') or norm_text.startswith('section') or norm_text.startswith('chapter') or norm_text.startswith('12.') or norm_text.startswith('14.'))):
                reading_mode = None
            ...
            for row in table.rows:
                for cell in row.cells:
                    ids = extract_prs_ids(cell.text)
                    for id_ in ids:
                        if reading_mode == 'matrix' or reading_mode == 'annexes':
...
def run_comparison():
    ...
    if word_file and excel_file:
        if st.button("🔍 Run Comparison", key="run_urs_tm"):
            ...
            st.write("### PRS DOC x Labeling Specification DOC Comparison")
            ...
            if psre_file is not None:
                ...
                st.write("### PRS DOC x PSRE Comparison")
            ...
            if rmm_file is not None:
                ...
                st.write("### PRS DOC x RMM Comparison")
