...
def run_comparison():    
    st.markdown("### Verification Test Protocol Revision Comparison")
    st.info("Upload your Verification Test Protocol Revision files to compare.")
    col1, col2 = st.columns(2)
    with col1:
        tp_file = st.file_uploader("Upload Verification Test Protocol Revision (.xlsx)", type=["xlsx"], key="tp_ver_rec")
    with col2:
        tr_file = st.file_uploader("Upload Verification Test Protocol Revision (.xlsx)", type=["xlsx"], key="tr_ver_rec")
    if tp_file and tr_file:
        if st.button("🔍 Run Comparison", key="run_ver_tp_rec"):
            ...
            protocol_new_sheets = ["Test Case Report - URS MD", "Test Case Report - URS NMD"]
            ...
            protocol_tc, protocol_prs, protocol_desc, protocol_step, protocol_exp = get_combined_cols(tp_path, protocol_sheets, 'Test Case ', 'Traceability (PRS)', 'Brief Description', 'Action / Step (Description)', 'Expected Result (Description)')
            records_tc, records_prs, records_desc, records_step, records_exp = get_combined_cols(tr_path, protocol_new_sheets, 'Test Case ', 'Traceability (PRS)', 'Brief Description', 'Action / Step (Description)', 'Expected Result (Description)')
            protocol_tc = normalize_spaces(filter_ignored(protocol_tc))
            protocol_prs = normalize_spaces(filter_ignored(protocol_prs))
            records_tc = normalize_spaces(filter_ignored(records_tc))
            records_prs = normalize_spaces(filter_ignored(records_prs))
            protocol_desc = normalize_spaces(filter_ignored(protocol_desc))
            records_desc = normalize_spaces(filter_ignored(records_desc))
            protocol_step = normalize_spaces(filter_ignored(protocol_step))
            records_step = normalize_spaces(filter_ignored(records_step))
            protocol_exp = normalize_spaces(filter_ignored(protocol_exp))
            records_exp = normalize_spaces(filter_ignored(records_exp))
            ...
            records_df = get_combined_df(
                tr_path, protocol_new_sheets,
                'Test Case ', 'Traceability (PRS)', 'Brief Description', 'Action / Step (Description)', 'Expected Result (Description)'
            )
            records_df['Test Case'] = normalize_spaces(filter_ignored(records_df['Test Case']))
            records_df['PRS'] = normalize_spaces(filter_ignored(records_df['PRS']))
            tc_results = {
                'VER Protocol Only in Protocol': sorted(set(protocol_tc) - set(records_tc)),
                'VER Protocol Only in Records': sorted(set(records_tc) - set(protocol_tc)),
                'VER Protocol Common to Both': sorted(set(protocol_tc) & set(records_tc))
            }
            prs_results = {
                'VER PRS Only in Protocol': sorted(set(protocol_prs) - set(records_prs)),
                'VER PRS Only in Records': sorted(set(records_prs) - set(protocol_prs)),
                'VER PRS Common to Both': sorted(set(protocol_prs) & set(records_prs))
            }
            desc_results = {
                'Brief Description Only in Protocol': sorted(set(protocol_desc) - set(records_desc)),
                'Brief Description Only in Records': sorted(set(records_desc) - set(protocol_desc)),
                'Brief Description Common to Both': sorted(set(protocol_desc) & set(records_desc))
            }
            step_results = {
                'Action / Step Only in Protocol': sorted(set(protocol_step) - set(records_step)),
                'Action / Step Only in Records': sorted(set(records_step) - set(protocol_step)),
                'Action / Step Common to Both': sorted(set(protocol_step) & set(records_step))
            }
            exp_results = {
                'Expected Result Only in Protocol': sorted(set(protocol_exp) - set(records_exp)),
                'Expected Result Only in Records': sorted(set(records_exp) - set(protocol_exp)),
                'Expected Result Common to Both': sorted(set(protocol_exp) & set(records_exp))
            }
            ...
            prs_only_in_protocol = force_to_strings(sorted(set(protocol_prs) - set(records_prs)))
            shown_tc_1 = set()
            shown_prs_1 = set()
            if prs_only_in_protocol:
                prs_to_tc = {}
                for prs in force_to_strings(prs_only_in_protocol):
                    linked_tcs = [tc for tc, p in zip(protocol_tc, protocol_prs) if p == prs]
                ...
                if 'Test Case Linked' in table_df_1.columns:
                    ...
                    total_tc_linked = 0
                st.markdown(
                    f"""Updated the Product verification test protocol spreadsheet attached in windchill to remove from scope the <b>{total_tc_linked}</b> Test Cases linked to the <b>{total_prs}</b> PRSs inactivated that were removed from the scope of verification 5.XX:""",
            ...
            protocols_only_in_protocol = force_to_strings(sorted(set(protocol_tc) - set(records_tc)))
            ...
            if protocols_only_in_protocol:
                for prot in force_to_strings(protocols_only_in_protocol):
                    linked_prs = [p for tc, p in zip(protocol_tc, protocol_prs) if tc == prot]
                ...
                total_protocols = len(protocol_tc)
                total_protocols_only = len(protocol_to_prs)
                total_prs_linked = len(set([row['PRS Linked'] for row in table_data_2]))

                st.markdown(
                    f"""Updated the Product verification test protocol spreadsheet attached in windchill to remove from scope the <b>{total_protocols_only}</b> Test Cases linked to the <b>{total_prs_linked}</b> PRSs that were removed from the scope of the product. This was done to improve the test coverage and writing of test cases and the test cases listed below were inactivated:""",
                    unsafe_allow_html=True
                )
                if table_df_2 is not None and not table_df_2.empty:
            ...
            prs_only_in_records = force_to_strings(sorted(set(records_prs) - set(protocol_prs)))
            shown_tc_2 = set()
            shown_prs_2 = set()
            if prs_only_in_records:
                prs_to_tc = {}
                for prs in force_to_strings(prs_only_in_records):
                    linked_tcs = [tc for tc, p in zip(records_tc, records_prs) if p == prs]
                    linked_tcs = force_to_strings(linked_tcs)
                    prs_to_tc[prs] = linked_tcs
                ...
                # Exibir o texto explicativo antes do DataFrame (apenas uma vez)
                ...
                if 'Test Case Linked' in table_df_3.columns:
                    ...
                    total_tc_linked = 0
                st.markdown(
                    f"""Updated the Product verification test protocol spreadsheet attached in windchill to include the <b>{total_tc_linked}</b> Test cases linked to the <b>{total_prs}</b> PRSs that were added in the scope of verification 5.XX""",
                    unsafe_allow_html=True
                )
            ...
            protocols_only_in_records = force_to_strings(sorted(set(records_tc) - set(protocol_tc)))
            ...
            if protocols_only_in_records:
                for prot in force_to_strings(protocols_only_in_records):
                    linked_prs = [p for tc, p in zip(records_tc, records_prs) if tc == prot]
                ...
                if table_df_4 is not None and not table_df_4.empty:
            ...
            tc_results = {
                ...
                'Test Case Common to Both': sorted(set(protocol_tc) & set(records_tc))
            }
            prs_results = {
                'Product Requirements Only in Protocol': prs_only_in_protocol,
                'Product Requirements Only in Records': prs_only_in_records,
                'Product Requirements Common to Both': sorted(set(protocol_prs) & set(records_prs))
            }
            desc_results = {
                'Brief Description Only in Protocol': sorted(set(protocol_desc) - set(records_desc)),
                'Brief Description Only in Records': sorted(set(records_desc) - set(protocol_desc)),
            ...
            try:
                ...
                with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                    ...
                    step_df.to_excel(writer, sheet_name='Action_Step_COMP', index=False)
                    exp_df.to_excel(writer, sheet_name='Expected_Result_COMP', index=False)
                    # Só exporta se não estiver vazio
                    if 'revision_mismatches' in locals() and not revision_mismatches.empty:
