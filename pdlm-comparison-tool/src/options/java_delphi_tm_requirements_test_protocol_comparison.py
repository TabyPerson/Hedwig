...
def run_comparison():       
    st.markdown("### Verification TM Requirements x Test Protocol Comparison")
    st.info("Upload your TM Requirements and Verification Test Protocol files to compare test protocols.")
    ...
    if tm_file and tp_file:
        if st.button("🔍 Run Comparison", key="run_ver_tp_tm"):
            ...
            tm_sheet = get_sheet_with_fallback(xl_tm, 'Traceability Matrix')
            ...
            # Read the 'Test Case Report' sheet from Verification Test Protocol, robust to spaces
            xl_tp = pd.ExcelFile(tp_path)
            # Find the sheet name that matches 'Test Case Report' ignoring leading/trailing spaces
            protocol_sheet = None
            for s in xl_tp.sheet_names:
                if s.strip().lower() == 'test case report':
                    protocol_sheet = s
                    break
            if protocol_sheet:
                df = pd.read_excel(tp_path, sheet_name=protocol_sheet)
                if "Test Case " in df.columns and "Traceability (PRS)" in df.columns:
                    TPcol = df["Test Case "].dropna().astype(str).str.strip()
                    TPprs = df["Traceability (PRS)"].dropna().astype(str).str.strip()
                else:
                    st.warning(f"Sheet '{protocol_sheet}' does not contain required columns 'Test Case ' and 'Traceability (PRS)'. Found: {df.columns.tolist()}")
                    TPcol = pd.Series(dtype=str)
                    TPprs = pd.Series(dtype=str)
            else:
                st.warning("Sheet 'Test Case Report' (with or without spaces) not found in the uploaded Verification Test Protocol file.")
                TPcol = pd.Series(dtype=str)
                TPprs = pd.Series(dtype=str)
            ...
            tc_results = {
                'Verification Test Case Only in Requirements Traceability': sorted(set(TMcol) - set(TPcol)),
                'VerificationTest Case Only in Verification Test Protocol': sorted(set(TPcol) - set(TMcol)),
                'Verification Test Case Common to Both Documents': sorted(set(TMcol) & set(TPcol))
            }
            # PRS comparison
            prs_results = {
                'Product Requirements Only in Requirements Traceability': sorted(set(TMprs) - set(TPprs)),
            ...
            st.download_button(
                "⬇️ Download Excel",
                data=buffer,
                file_name="VER_TPxTM_T_comparison_summary.xlsx",
